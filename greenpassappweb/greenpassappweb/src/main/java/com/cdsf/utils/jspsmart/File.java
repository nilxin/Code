...
public class File
{
    ...
    private String m_fileSecretKey;
    ...
    File()
    {
        ...
        m_fileSecretKey=String.valueOf(random.nextDouble()*100000).substring(0, 5);
    }
    ...
    //jiang-可以加密文件
    public void saveAs(String s,int m,boolean aesEncrypt) throws IOException,SmartUploadException
    {
        String s1 = "";
        s1 = m_parent.getPhysicalPath(s,m);
        if(s1 == null)
        {
            throw new IllegalArgumentException("There is no specified destination file (1140).");
        }
        try
        {
            ...
            if(aesEncrypt){
            	Cipher cipher =m_parent.initEncryptor(m_fileSecretKey, Cipher.ENCRYPT_MODE);
                byte[] bytes;
                bytes = cipher.doFinal(m_parent.m_binArray,m_startData,m_size);
                fos.write(bytes);
            }else{
            	fos.write(m_parent.m_binArray,m_startData,m_size);
            }
            fos.flush();
            fos.close();
        }
