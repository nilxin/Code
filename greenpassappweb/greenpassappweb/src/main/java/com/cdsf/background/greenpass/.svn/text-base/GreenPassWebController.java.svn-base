...
public class GreenPassWebController extends AbstractController {
	...
	public Map<String, Object> auditStationStatistics2(@RequestModel Map<String, Object> map,@RequestModel("page") Pageable page) throws Exception {
		Map<String, Object> result = new HashMap<String, Object>();
		dealTimeCondition(map);
		if(Tools.isNotNull(map.get("auditStation")) &&  map.get("auditStation").equals("-1")){
			map.put("auditStation", null);
		}else if(Tools.isNotNull(map.get("auditStation")) &&  map.get("auditStation").equals("1")){
			map.put("auditStation", 1);
		}else if(Tools.isNotNull(map.get("auditStation")) &&  map.get("auditStation").equals("2")){
			map.put("auditStation", 2);
		}
		try{
	...
	public Map<String, Object> cargoStatistic(@RequestModel Map<String, Object> map,@RequestModel("page") Pageable page) throws Exception {
		...
		if(Tools.isNotNull(map.get("firstCargoCondition"))){
			map.put("firstCargoCondition", map.get("firstCargoCondition").toString());
		}else{
			map.put("firstCargoCondition", "");
		}
		try{
...
//			List<?> violationTruckPlateList = greenPassWebService.queryCheckTimeStatistics(map,page);
			List<?> violationTruckPlateList = greenPassWebService.queryCheckTimeStatisticsByHours(map, page);
			result.put("status", 1);
			result.put("content", violationTruckPlateList);
			result.put("message", MessageConstants.SUCCESS);
		}catch(Exception e){
	...
	public Map<String, Object> doubleCheckStatus(String billId,String doubleCheckStatus,String currentUserWorkNo) throws Exception {
		
		Map<String, Object> result = new HashMap<String, Object>();
		try{
			greenPassWebService.updateDoubleCheckStatus(billId,doubleCheckStatus,currentUserWorkNo);
			result.put("status", 1);
			result.put("content", "");
			result.put("message", MessageConstants.SUCCESS);
		}catch(Exception e){
